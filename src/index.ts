...
import { routesList, routesCache, routesClear } from "./tools/routes.js";
...
async function main(): Promise<void> {
  ...
  registerResources(mcpServer.server);

  const transport = new StdioServerTransport();
  await mcpServer.connect(transport);

  process.stderr.write(
    `Laravel Codebase MCP started | project: ${PROJECT_PATH}\n`
  );
}
